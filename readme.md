...
> 📄 **[Read the Full Project Report Here](docs/GS_decoder_implemention.pdf)** This repository provides a complete implementation and experimental analysis of the Guruswami-Sudan (GS) list decoding algorithm for Reed-Solomon (RS) codes. The implementation is written in SageMath to leverage its robust algebraic capabilities over finite fields. 
...
* **Software:** [SageMath](https://www.sagemath.org/) (Version 10.7 or compatible) 
  * *Ubuntu/Debian Installation:* `sudo apt install sagemath`
...
* `docs/`: Directory containing the full project report.

**Assets:**
* `experiment1_logs.txt`: Generated log outputs from the execution of the first experiment.
* `exp1_radius.png`: Visualizations of the success probability against the number of errors with multiplicity $m=1$.
* `exp1_radius_comparison.png`: Visualizations of the success probability against the number of errors across varying multiplicities $m$.
...
1. **Decoding Radius vs. Multiplicity (Exp 1):** Increasing $m$ monotonically increases the error-correction capability past the unique decoding bound. For a code of $RS(64,16)$, $m=1$ corrects 27 errors, and $m=4$ corrects 31 errors (unique bound is 24). 
   ...
   ![Decoding Radius Comparison](exp1_radius_comparison.png) 
...
2. **Runtime Complexity (Exp 2):** The interpolation matrix growth introduces a steep computational cost. Runtime scales non-linearly, approaching $O(m^6)$, making smaller multiplicities ($m=2$ or $m=3$) more practical for real-time applications.
   ...
   ![Runtime Complexity](exp2_runtime.png)
...
3. **List Size Behavior (Exp 3 & 4):** Despite being a "list" decoder, random symmetric noise almost universally produces a list size of exactly 1.
   ...
   ![List Size Behavior](exp3_listsize.png)
...
You can run the main simulation block by executing the Sage script directly in your terminal. The bottom of the `RS.sage` file contains the execution block for the experiment suite:

```bash
sage RS.sage
```
## References
V. Guruswami, A. Rudra, and M. Sudan, Essential Coding Theory, Draft avail-able at https://cse.buffalo.edu/faculty/atri/courses/coding-theory/book/web-coding-book.pdf, 2025. (Chapters 12-13)

## Authors
This implementation was developed as a 3rd-year semester project. 
Code Implementation: Nadav Cremisi
Project Report: Nadav Cremisi and Ori Kremer
